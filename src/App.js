...
function App() {
  ...
  const fetchAPI = async () => {
    const data1 = await fetch("https://mustang-index.azurewebsites.net/index.json")
      .then((res) => res.json()
  ...
  // handle the array contents 
  // get contact URLs
  const getArray = (dataArray) => {
    const copyList = []; 
    dataArray.forEach((element) => { 
      copyList.push(element.ContactURL); // append the elements
    });

    // fetch all JSON data from URLs
    Promise.all(copyList.map(
      (urls) => fetch(urls)
          .then((res) => res.json())
          .catch((err) => console.log(err))
      )
